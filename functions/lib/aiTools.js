...
exports.INJURY_MAP = void 0;
exports.parseInjuries = parseInjuries;
exports.resolveDifficulty = resolveDifficulty;
exports.buildWeightGuidance = buildWeightGuidance;
exports.buildInjuryReinforcement = buildInjuryReinforcement;
exports.buildSystemPrompt = buildSystemPrompt;
exports.buildUserPrompt = buildUserPrompt;
exports.validatePlanSafety = validatePlanSafety;
exports.validateWeightFormat = validateWeightFormat;
exports.cacheKeyContributors = cacheKeyContributors;
// ---------------------------------------------------------------------------
...
// Injury parsing
// ---------------------------------------------------------------------------
function parseInjuries(freeText) {
    if (!freeText || typeof freeText !== "string")
        return [];
    const text = freeText.toLowerCase();
    const matched = new Set();
    const tags = [];
    for (const [id, rule] of Object.entries(exports.INJURY_MAP)) {
        if (matched.has(id))
            continue;
...
// ---------------------------------------------------------------------------
function resolveDifficulty(questionnaireLevel, gamificationLevel) {
    const self = normalizeLevel(questionnaireLevel);
    const lvl = typeof gamificationLevel === "number" ? gamificationLevel : 1;
    if (self === "beginner" && lvl >= 11)
        return "intermediate";
    if (self === "intermediate" && lvl >= 21)
        return "advanced";
    if (self === "advanced" && lvl <= 5)
        return "intermediate";
    return self;
}
...
function buildWeightGuidance(metrics, difficulty, sex) {
    ...
    const round = (n) => Math.max(2.5, Math.round((n / 2.5)) * 2.5);
...
// ---------------------------------------------------------------------------
function buildInjuryReinforcement(tags) {
    if (tags.length === 0)
        return "";
    const lines = [
        "",
...
// ---------------------------------------------------------------------------
function buildUserPrompt(questionnaire, metrics, recentWorkouts, injuryTags, difficulty, questionnaireLevelRaw, gamificationLevel) {
    const parts = [];
...
function validatePlanSafety(plan, injuryTags) {
...
// ---------------------------------------------------------------------------
function validateWeightFormat(plan) {
    const warnings = [];
    for (const day of plan.weeklyPlan) {
        for (const ex of day.exercises) {
...
// ---------------------------------------------------------------------------
function cacheKeyContributors(questionnaire, metrics, difficulty, injuryTags) {
    return {
